package mx.com.sky.sel.managedbeans.compradatos;
...
public class BBCompraDatos extends UtileriasMB implements Serializable {
    @SuppressWarnings("compatibility:-570183896729821995")
    private static final long serialVersionUID = 7561031801669491310L;

    private List<PaqueteDatos> mosaicoPaquetesDatos = null;
    ...
    private Boolean isSuplementario;
    ...
    public BBCompraDatos() {
        System.out.println("BBCompraDatos creado...........................");
        consultaSaldo();
        obtenerCPaquetes();
        limpiarDatos();
    ...
    private void obtenerCPaquetes() {
        ConfigMenu sesion = (ConfigMenu)JSFUtils.getBean("sesion");
        if(sesion.getSuscriptor().getAssetNumberInternet() == null){
            LogPC.println(this, "AssetNumberInternet null");
            sinPaquetes = true;
            return;
        }
        mosaicoPaquetesDatos = new ArrayList<PaqueteDatos>();
        this.paquetes = new PaqueteDatos[20];
        
        ServicioCompraDatos servCompraDatos = new ServicioCompraDatos();
        mosaicoPaquetesDatos = servCompraDatos.consultarPaqueteDatos(sesion.getSuscriptor().getCuentaSKY(), sesion.getSuscriptor().getAssetNumberInternet(), sesion.getSuscriptor().getSecondAssetNumberInternet(),
                                                                            sesion.getSuscriptor().getPais());
        
        if (mosaicoPaquetesDatos == null || mosaicoPaquetesDatos.isEmpty()) {
            LogPC.println(this, "paquetes de internet null");
            sinPaquetes = true;
        } else {
            sinPaquetes = false;

            for (int i = 0; i < mosaicoPaquetesDatos.size(); i++){
                this.paquetes[i] = mosaicoPaquetesDatos.get(i);
                this.paquetes[i].setSaldoCuenta(saldoActual);
            }
                LogPC.println(this, "[contadorPaquete][" + mosaicoPaquetesDatos.size() + "]");
            
        }
        OperacionBAMUtils.registrarOperacion(EnumOperacionBAM.CONSULTA_PAQUETES_DATOS);
    }
    ...
    public String cbContrataDatosTyA_action() {
        ...
        respuestaCompra = compraDatos.comprarPaqueteDatos(sesion.getSuscriptor().getCuentaSKY(), this.assetNumber, this.secondAssetNumber,
        ...
        OperacionBAMUtils.registrarOperacion(EnumOperacionBAM.COMPRA_PAQUETES_DATOS, "Compra datos: "+idPaquete);
        /*System.out.println(this.assetNumber);
        System.out.println("secondAN: ==" + this.secondAssetNumber + "==");*/
        return "ok";
    }
    ...
    public String generarNombreArchivoPaises(){
        return Normalizer.normalize(this.paqueteActiva.getNombre_display(), Normalizer.Form.NFD).replaceAll("[^A-Za-z0-9]", "").toUpperCase();
    }
    
    public void setMosaicoPaquetesDatos(List<PaqueteDatos> mosaicoPaquetesDatos) {
        this.mosaicoPaquetesDatos = mosaicoPaquetesDatos;
    }

    public List<PaqueteDatos> getMosaicoPaquetesDatos() {
        return mosaicoPaquetesDatos;
    }
    ...
    public String getSecondAssetNumber() {
        return this.secondAssetNumber;
    }

    public void setIsSuplementario(Boolean isSuplementario) {
        this.isSuplementario = isSuplementario;
    }

    public Boolean getIsSuplementario() {
        return isSuplementario;
    }
    ...
    */
    ...
    public Boolean getMostrarPaisesIncluidos() {
